...
<div class="container" style="margin-top: 5rem; margin-bottom: 8rem;">
    ...
    @foreach ($concurso->vagas as $i => $area)
        <div class="row justify-content-center" style="margin-bottom: 20px;">
            <div class="col-md-11">
                <div class="card shadow bg-white style_card_container">
                    <div class="card-header d-flex justify-content-between bg-white" id="style_card_container_header">
                        <div class="form-group">
                            <h6 class="style_card_container_header_titulo">Banca examinadora do(a) {{$concurso->titulo}} - {{$area->nome}}</h6>
                            <h6 class="" style="font-weight: normal; color: #909090; margin-top: -10px; margin-bottom: -15px;">Meus concursos > Banca examinadora - {{$area->nome}}</h6>
                        </div>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                                <a id="selecionar-user" class="dropdown-item" href="#" data-toggle="modal" data-target="#presidente-banca{{$area->id}}" >Selecionar presidente da banca</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <table class="table table-bordered table-hover tabela_container table-responsove-md">
                            ...
                            <tbody >
                                @if(isset($membrosBancas[$area->id]) == false || $membrosBancas[$area->id]->first() == null || $membrosBancas[$area->id]->first()->vaga_id != $area->id)
                                    <td id="tabela_container_linha">
                                        Sem usuários cadastrados ainda
                                    </td>
                                @else
                                    @forelse ($membrosBancas[$area->id] as $membro)
                                        <tr>
                                            <td id="tabela_container_linha">
                                                {{ $membro->user->nome }}
                                            </td>
                                            <td id="tabela_container_linha">
                                                {{ $membro->user->sobrenome }}
                                            </td>
                                            <td id="tabela_container_linha">
                                                {{ $membro->user->email }}
                                            </td>
                                            ...
                                            <td id="tabela_container_linha" style="text-align: center;">
                                                <div class="btn-group">
                                                    <div>
                                                        <button class="btn btn-danger"onclick ="this.disabled=true;location.href='{{ route('concurso.remover.banca', ['user' => $membro->user->id, 'concurso' => $concurso->id, 'vaga' => $area->id]) }}'">
                                                            Remover da banca
                                                        </button>
                                                    </div>
                                                </div>
                            ...
                            @forelse ($usuariosMembros as $user)
                                <tr>
                                    ...
                                    <td id="tabela_container_linha" style="text-align: center;">
                                        <div class="btn-group">
                                            <div>
                                                <button id="atribuir-membro" class="btn btn-info" data-toggle="modal" data-target="#membro-banca" onclick="vagasBanca({{$user->id}}, {{$concurso->id}}, '{{$user->nome.' '.$user->sobrenome}}')">Adicionar a banca</button>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
...
@foreach ($concurso->vagas as $i => $area)
    <div class="modal fade" id="presidente-banca{{$area->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Selecione o presidente da banca examinadora para a área {{$area->nome}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            </div>
            <div class="modal-body">
                <form id="selecionar-presidente-form{{$area->id}}" method="POST" action="{{route('user.presidente.banca', ['concurso' => $concurso->id, 'vaga' => $area->id])}}">
                    @csrf
                    <div class="col-md-12 form-group">
                        <input type="hidden" name="concurso" value="{{$concurso->id}}">
                        <label for="presidente">Presidente</label>
                        <select name="presidente"  class="form-control" id="presidente" required>
                            @if(isset($membrosBancas[$area->id]) == false || $membrosBancas[$area->id]->first() == null)
                                <option value="" selected disabled>-- Nenhum membro adicionado a banca --</option>
                            @else
                                <option value="" selected disabled>-- Selecione o presidente --</option>
                                @foreach ($membrosBancas[$area->id] as $membro)
                                    <option value="{{$membro->user->id}}">{{$membro->user->nome . " " . $membro->user->sobrenome}}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="col-md-12 form-group" style="margin-bottom: 9px;">
                        <button class="btn btn-success shadow-sm" style="width: 100%;" id="submeterFormBotao" form="selecionar-presidente-form{{$area->id}}">Salvar</button>
                    </div>
                </form>
            </div>
        </div>
...
<div class="modal fade" id="membro-banca" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title">Defina a(s) banca(s) para o membro</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            </div>
            <div class="modal-body">
                ...
                <form id="membro-banca-form" method="POST" action="{{route('concurso.adicionar.membro.banca')}}">
                    @csrf
                    <div class="col-md-12 form-group" id="selecaoMembro">
                        <input type="hidden" name="concurso" value="{{$concurso->id}}">
                        <input type="hidden" name="membro_error" value="0">
                        <input type="hidden" name="membro" id="membroInput" value="">
                        <label for="membro">Membro: </label>
                        <div id="nomeMembro" style="font-weight: normal">
...
<script>
    function vagasBanca(membroId, concurso, nome) {
        var user = membroId;

        var vagas = document.getElementById('vagas');
        vagas.innerHTML = "";
        document.getElementById("nomeMembro").innerHTML = nome;


        let element = document.getElementById('membroInput');
        element.value = membroId;
